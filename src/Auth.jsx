...
const saveUsers = (users) => localStorage.setItem("sw_users", JSON.stringify(users));
...
export default function Auth({ onLogin }) {
  ...
  const handleLogin = () => {
    ...
    setTimeout(() => {
      ...
      if (!user) {
        setLoading(false);
        setAuthError("❌ Account nahi mila! Pehle register karo yaar.");
        return;
      }
      if (user.password !== form.password) {
        setLoading(false);
        setAuthError("❌ Wrong password bhai! Dobara try karo.");
        return;
      }
  ...
  const handleRegisterSubmit = () => {
    ...
    setTimeout(() => {
      ...
      if (users[form.phone]) {
        setLoading(false);
        setAuthError("❌ Is phone number se pehle se account hai! Login karo.");
        return;
      }
          ...
          <div style={{ padding: "54px 24px 0" }}>
            ...
            <div className="fade-up">
              ...
              <div style={{ marginBottom: 16 }}>
                <div className="label">Phone ya Email</div>
                <div className="inp-wrap">
                  <span className="inp-icon">📱</span>
                  <input className={`inp ${errors.phone ? "err" : ""}`} placeholder="Phone number ya email daalo" value={form.phone} onChange={e => set("phone", e.target.value)} />
                </div>
